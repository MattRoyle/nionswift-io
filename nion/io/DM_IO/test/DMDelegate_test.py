...
class TestDMAbstract:
    class TestDMImportExportBase(abc.ABC, unittest.TestCase):
        """This is the abstract base class for testing share functionality of DMIODelegates

        It is nested in TestDMAbstract since otherwise unittest will try running these and fail because of the abstract
        methods.
        """
        @property
        @abc.abstractmethod
                            ...
                            for signal_type in (
                                ...
                                with array_type(shape, dtype) as a:
                                    with io.BytesIO() as s:
                                        ...
                                        self.dm_delegate._export_data_and_metadata(xdata_in, s, version)
                                        s.seek(0)
                                        xdata = self.dm_delegate._import_data_and_metadata(s)
