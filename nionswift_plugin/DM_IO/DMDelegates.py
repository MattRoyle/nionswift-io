...
class DMIODelegate(abc.ABC):
    ...
    def get_time_infomation(self, modified, timezone, timezone_offset) -> typing.Tuple[str | None, str | None, str | None]:
        timezone_str = None
        date_str = None
        time_str = None
        if modified:
        ...
        return timezone_str, date_str, time_str
...
class DM5IODelegate(DMIODelegate):
    ...
    def save_image(self, data_and_metadata: DataAndMetadata.DataAndMetadata, file: typing.BinaryIO, _: int) -> None:
        ...
        with (h5py.File(file, "w") as f):
            ...
            if unique_id:
                DM5Utils.convert_dict_to_group(unique_id, source_image) # Add the unique ID to the source image if there is one
            ...
            if dimensional_calibrations and len(dimensional_calibrations) == len(data.shape):
                ...
                for i, dimensional_calibration in enumerate(reversed(dimensional_calibrations)):
                    ...
                    DM5Utils.save_attr_to_group(name="Scale", data=dimensional_calibration.scale, dtype=numpy.float32, group=dimension)  # dm5 stores scale as a float32. This can introduce floating point issues as python uses 64-bit floats
            ...
            if intensity_calibration:
                ...
                DM5Utils.save_attr_to_group(name="Scale", data=intensity_calibration.scale, dtype=numpy.float32, group=brightness)
            ...
            convert_dict_to_group(metadata, image_tags) # The metadata dictionary is treated as the image tags
            ...
            DM5Utils.save_attr_to_group(name="ImageRef", data=1, group=image_source, dtype=numpy.uint32)  # The reference in the ImageList
            ...
            DM5Utils.save_attr_to_group(name="AnnotationType", data=20, group=data_document_object, dtype=numpy.uint32)  # Annotation type 20 is image display
            ...
            if datum_dimension_count == 1:
                # 1d data is always marked as spectrum
                DM5Utils.save_attr_to_group("Format", data="Spectrum image" if collection_dimension_count == 2 else "Spectrum", group=meta_data_group)
            if needs_slice or (collection_dimension_count + (1 if is_sequence else 0)) == 1:
                ...
                if needs_slice:
                    ...
                    DM5Utils.save_attr_to_group(name="Rectangle", data=(0, 0, 1, 1), group=annotation_group, dtype=[('top', '<f4'), ('left', '<f4'), ('bottom', '<f4'), ('right', '<f4')])
